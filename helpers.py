...
Although `datetime`s already have human-readable string representations,
those representations display seconds, but NASA's data (and our datetimes!)
don't provide that level of resolution, so the output format also will not.
"""
...
def cd_to_datetime(calendar_date):
    ...
    and time.
    """
    date_time = datetime.datetime.strptime(calendar_date, "%Y-%b-%d %H:%M")
...
def datetime_to_str(dt):
    ...
    month, date, hour, and minute values. Additionally, this function provides
